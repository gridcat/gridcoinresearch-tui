...
#
# Apply() in internal/selfupdate/apply.go refuses to download an asset larger than
...
#
# The cap is read out of apply.go rather than duplicated here, so raising
...
cap_mib=$(grep -oP 'maxDownloadBytes = \K\d+(?= << 20)' internal/selfupdate/apply.go || true)
if [ -z "$cap_mib" ]; then
  echo "::error::cannot parse maxDownloadBytes from internal/selfupdate/apply.go"
  exit 1
fi
cap=$((cap_mib << 20))
status=0
...
for target in linux/amd64 linux/arm64 darwin/amd64 darwin/arm64 windows/amd64; do
  ...
  CGO_ENABLED=0 GOOS=${target%/*} GOARCH=${target#*/} \
    go build -ldflags="-s -w" -o "$out" ./cmd/gridcoinresearch-tui
  size=$(stat -c %s "$out")
  rm -f "$out"
  printf '%-14s %s bytes (%s MiB) of %s MiB cap\n' "$target" "$size" \
    "$(LC_ALL=C awk -v b="$size" 'BEGIN{printf "%.1f", b/1048576}')" "$cap_mib"
  if [ "$size" -gt "$cap" ]; then
    echo "::error::$target binary is $size bytes, over the ${cap_mib} MiB maxDownloadBytes cap in internal/selfupdate/apply.go — raise the cap"
    status=1
  fi
done
