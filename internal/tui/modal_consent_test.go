...
func TestLaunchOnlyOptInGetsAReporterID(t *testing.T) {
	...
	m := NewModel(config.Config{PeerSharing: state.PeerSharingOn}, nil)
	if !m.sharingOn {
	...
	if got := state.Load().PeerSharing; got != state.PeerSharingUnset {
	...
	if state.Load().ReporterID != m.state.ReporterID {
...
func TestLaunchOnlyOptOutMintsNothing(t *testing.T) {
	...
	if m := NewModel(config.Config{PeerSharing: state.PeerSharingOff}, nil); m.state.ReporterID != "" {
...
func TestHeartbeatIsArmedRegardlessOfConsent(t *testing.T) {
	...
	for _, share := range []state.PeerSharing{state.PeerSharingOn, state.PeerSharingOff} {
		m := NewModel(config.Config{PeerSharing: share}, nil)
...
func TestBeatDoesNotReportWhenSharingIsOff(t *testing.T) {
	...
	m := Model{
		sharingOn:    false,
		state:        state.State{ReporterID: "a1b2c3d4e5f60718293a4b5c6d7e8f90"},
		nextReportAt: booked,
	}
...
func TestLaunchOnlyOptInActuallyDispatchesAReport(t *testing.T) {
	...
	m := NewModel(config.Config{PeerSharing: state.PeerSharingOn}, nil)
	m.peers = []rpc.PeerInfo{{Addr: "45.33.32.156:32749"}}
...
func TestAckAppliesCadenceToTheNextReport(t *testing.T) {
	...
	next, _ := m.Update(sharingDoneMsg{ack: telemetry.Ack{OK: true, Accepted: 3, NextReportAfter: 24 * 3600}})
	got := next.(Model)
	if got.sharingInterval != telemetry.MaxInterval {
		t.Errorf("interval = %v, want %v", got.sharingInterval, telemetry.MaxInterval)
	}
...
func TestSwitchingSharingOnArmsNoSecondTimer(t *testing.T) {
	...
	unchanged := NewModel(config.Config{}, nil)

	toggled := NewModel(config.Config{}, nil)
