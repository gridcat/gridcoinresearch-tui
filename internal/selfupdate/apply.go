...
// Apply downloads the release archive for this OS/arch, verifies its
...
func Apply(rel Release) (string, error) {
	archive := AssetName()
...
// run legitimately can't replace it, so say so instead of a bare
// "permission denied".
