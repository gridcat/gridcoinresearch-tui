...
// can read. Kept in one file so the render code stays focused on
...
//
// It exists because daemon-sourced strings do not all come from our own
...
func PollExpired(expiration string) bool {
	return PollExpiredAt(ParsePollTime(expiration))
}

// PollExpiredAt is the parsed-time form of PollExpired, so a caller that already
// has the parsed expiration (e.g. renderPollRow) doesn't re-parse the string.
func PollExpiredAt(exp time.Time) bool {
...
func FormatPollTimeLeft(expiration string) string {
	return PollTimeLeftAt(ParsePollTime(expiration))
}

// PollTimeLeftAt is the parsed-time form of FormatPollTimeLeft, so a caller
// with the expiration already parsed avoids a second time.Parse.
func PollTimeLeftAt(exp time.Time) string {
	if exp.IsZero() {
		return "—"
	}
...
func IsContractCandidate(tx rpc.Transaction) bool {
...
func ClassifyTransaction(tx rpc.Transaction) TxStatus {
