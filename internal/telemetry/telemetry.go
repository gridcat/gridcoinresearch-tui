...
// modal_consent.go promises exactly this shape. Widening it later means revisiting
...
// Report is the exact JSON body posted to the collector. Addresses and
...
// Ack is the collector's answer. NextReportAfter lets the server pace
...
// to test.
func shareablePeers(peers []rpc.PeerInfo) []string {
...
// Schedule decides what one heartbeat beat should do: which slot to book
...
func Schedule(now, booked time.Time, interval time.Duration) (next time.Time, send bool) {
	if interval <= 0 {
		interval = Interval
	}
	switch {
...
// Build assembles the payload. It returns ok=false when there is
// nothing worth sending, so callers can skip the request entirely rather than
// posting an empty peer list.
func Build(peers []rpc.PeerInfo, testnet bool, reporterID string) (Report, bool) {
	...
	return Report{
		...
		Reporter: reporterID,
		Client:   "gridcoinresearch-tui/" + buildinfo.Version,
