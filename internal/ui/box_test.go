...
func TestTitledBoxTopEdge(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			out := TitledBox(boxStyleOfWidth(tc.width), theme.Title, "Transactions", "x")
...
// a negative count, since a panic there takes the whole TUI down mid-render.
func TestTitledBoxWideGlyphTitle(t *testing.T) {
	for _, width := range []int{4, 5, 7, 10, 20, 40} {
		style := boxStyleOfWidth(width)
		out := TitledBox(style, theme.Title, "❤️❤️❤️ hearts", "x")
		top := strings.SplitN(out, "\n", 2)[0]
...
func TestTitledBoxOuterSizeMatchesPlainBox(t *testing.T) {
	...
	for _, height := range []int{3, 5, 8} {
		...
		titled := TitledBox(style, theme.Title, "Transactions", content)
...
func TestTitledBoxTitleFollowsFocus(t *testing.T) {
	...
	idle := TitledBox(style, theme.Title, "Transactions", "row")
	focused := TitledBox(theme.BorderFocused.Padding(0, 1).Width(30), theme.Accent, "Transactions", "row")

	if !strings.Contains(idle, theme.Title.Render(" Transactions ")) {
		t.Error("idle box does not carry a styleTitle label")
	}
	if !strings.Contains(focused, theme.Accent.Render(" Transactions ")) {
