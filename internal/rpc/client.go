...
// Client is the object the rest of the program uses to talk to the daemon.
...
// NewClient builds a fresh client from a resolved Config. The returned
// value is a pointer (*Client) so mutation of the counter persists across
// calls and so copying the client doesn't duplicate the HTTP connection pool.
// Options is everything the client needs to reach a daemon. It exists so this
// package stays free of the app's Config type: config resolution (flags, env,
// gridcoinresearch.conf) is a separate concern from speaking JSON-RPC.
type Options struct {
	URL      string
	User     string
	Password string
...
// use CamelCase field names, but JSON wants lowercase, and the tags bridge that.
...
// a hostile or impersonated daemon can't OOM the process with a multi-GB
// body, the same guard selfupdate.go's maxDownloadBytes applies to release
