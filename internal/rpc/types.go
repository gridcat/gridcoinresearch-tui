...
// lowercase, and the tag bridges the two conventions.
package rpc
...
// currently unlocked. An unencrypted wallet (UnlockedUntil == nil) and one
// the user has already unlocked themselves (e.g. for staking) both return
// false, so callers don't ask for a passphrase the daemon doesn't need.
...
type StakingInfo struct {
	...
	// configured (crunchers) and omits them entirely for investors. They are
...
// against any wallet version, which is what the custom UnmarshalJSON below does.
...
// without the private key. The daemon tracks balances on them but cannot
// sign on their behalf. The field is only present in the JSON
